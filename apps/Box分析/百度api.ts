...
export class Box extends plugin {
  ...
  async Box(e) {
    ...
    const data = await global.clientApiByQQ.messageApi.message(
      channelID,
      messageID
    )
    ...
    async function main() {
      ...
      function levenshteinDistance(str1: string, str2: string): number {
        const m = str1.length
        const n = str2.length
        const dp: number[][] = Array.from(Array(m + 1), () =>
          Array(n + 1).fill(0)
        )
      ...
      let matchingNames: string[] // 在函数外部定义
      // 定义一个函数，用于找到最相似的角色名字
      ...
      if (response.data && Array.isArray(response.data.words_result)) {
        const matchingNames = response.data.words_result
          .filter(
            (wordResult: { words: any }) => wordResult && wordResult.words
          ) // 过滤掉未定义或没有 words 属性的项
          .map((wordResult: { words: string }) => wordResult.words.trim()) // 对筛选后的项进行 map 操作
          .map((word: string) => findBestMatchingName(word)) // 使用函数查找最相似的名字
          .filter((name: any) => name !== undefined) as string[] // 过滤掉 undefined 的结果
      ...
      function findMatchingTeams(names: string[]): TeamsData[] {
        ...
        for (const teamName in teamsData) {
          ...
          if (Array.isArray(names) && Array.isArray(teamCharacters)) {
            const foundCharacters = teamCharacters.filter(name =>
              names.includes(name)
            )
            // 继续处理 foundCharacters
