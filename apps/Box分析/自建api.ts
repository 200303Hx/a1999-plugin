...
export class Box1 extends plugin {
  constructor() {
    super({
      rule: [
        {
  ...
  async Box2(e: Messagetype) {
    const OCR = /^自建ocr\s+([\S]+)/
    const match = e.cmd_msg.match(OCR)
  ...
  async Box1(e: Messagetype) {
    ...
    const OCR = parsedData.partOfUrl

    const remoteFileUrl = img // 远程文件的 URL
    const localFilePath = `${process
      .cwd()
      .replace(/\\/g, '/')}/plugins/alemon-plugin-1999/resources/assets/img/Box/Box.jpg` // 本地保存文件的路径
    const trwebocr = 'http://'
    const apiurl = trwebocr + OCR

    axios.get(remoteFileUrl, { responseType: 'stream' }).then(response => {
      response.data.pipe(fs.createWriteStream(localFilePath)).on('finish', () => {
        // 下载完成后，开始上传文件
        ...
        function uploadFile(filePath) {
          ...
          axios
            ...
            .then(response => {
              ...
              const matchingNames = response.data.data['raw_out']
                .map(wordResult => wordResult[1]) // 获取第二个元素，即名字字符串
                .map(word => findBestMatchingName(word.trim())) // 使用函数查找最相似的名字
