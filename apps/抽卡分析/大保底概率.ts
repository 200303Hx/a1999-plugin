    ...
    async function analyzeAndDisplayStats() {
      ...
      file2Data.forEach(pool => {
        ...
        Object.keys(poolStats).forEach(poolName => {
          const { sixStarPulls, misfitPulls } = poolStats[poolName]
          misfitCount += misfitPulls
          notMisfitCount += sixStarPulls - misfitPulls
        })
        // 根据不同情况计算歪卡的概率
