...
export class fenxichouka2 extends plugin {
  ...
  async fenxi2(e) {
    ...
    const extensionImagePath = `./plugins/a1999-plugin/resources/assets/img/抽卡分析/分析/pinjie.jpg` // 请替换为实际路径
    Jimp.read(extensionImagePath, async (err, extensionImage) => {
      ...
      const newFilePath = `${process
        ...
        .replace(
          /\\/g,
          '/'
        )}/plugins/a1999-plugin/db/抽卡分析/抽卡记录2${userId}.json`
      ...
      // 处理限定池数据
      const limitedPoolData = processGachaPoolData(
        newFileData,
        '限定池（其余卡池）'
      )
      // 处理常驻池数据
      const standardPoolData = processGachaPoolData(
        newFileData,
        '常驻池（于湖中央）'
      )
      // 处理新手池数据
      const novicePoolData = processGachaPoolData(
        newFileData,
        '新手池（滴一滴雨）'
      )
      ...
      Jimp.read(imageFilePath, async (err, image) => {
        ...
        if (textHeight + 710 >= 2000) {
          ...
          for (let i = 0; i < numExtensions; i++) {
            const extensionImageY =
              backgroundImageHeight + i * extensionImageHeight
          ...
          // 保存修改后的图片
          const outputImagePath = `./plugins/a1999-plugin/resources/assets/img/抽卡分析/分析/choukafenxi.jpg` // 请替换为实际路径
          backgroundImage2.write(outputImagePath, async writeErr => {
            ...
            e.reply(
              getPathBuffer(
                `./plugins/a1999-plugin/resources/assets/img/抽卡分析/分析/choukafenxi.jpg`
              )
            )
          })
        } else {
