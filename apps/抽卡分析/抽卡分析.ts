  ...
  async fenxi2(e: Messagetype) {
    ...
    const extensionImagePath = `${process
      ...
      .replace(
        /\\/g,
        '/'
      )}/plugins/alemon-plugin-1999/resources/assets/img/抽卡分析/分析/pinjie.jpg` // 请替换为实际路径
    Jimp.read(extensionImagePath, async (err, extensionImage) => {
      ...
      const fontPath = `${process
        ...
        .replace(
          /\\/g,
          '/'
        )}/plugins/alemon-plugin-1999/resources/assets/ttf/SourceHanSerifSC-VF.ttf`
        ...
        if (textHeight + 710 >= 2000) {
          ...
          const outputImagePath = `${process
            ...
            .replace(
              /\\/g,
              '/'
            )}/plugins/alemon-plugin-1999/resources/assets/img/抽卡分析/分析/choukafenxi.jpg` // 请替换为实际路径
